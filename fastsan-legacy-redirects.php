 ...
 * 1.2.0 (C, 2026-09-08): ägarval (a) — ETT mål för kategori-URL:er. /sv/category/insikter/ pekade till arkivet /kategori/insikter/, som aib-robots sätter noindex på; 301:an landade alltså i en död ände medan /category/insikter/ gick till sidan /insikter/. Nu går båda baserna till den redaktionella hubben via fastsan_legacy_category_target(). Okända slugs behåller /kategori/X/ som förut. Arkivet självt rörs inte (noindex,follow kvar).
...
/**
 * 1.2.0 — one target per category. Legacy category URLs go to the editorial hub
 * page, not to the (noindex) term archive. Unknown slugs keep the archive path.
 *
 * @param string $rest Path fragment after the category base, e.g. 'insikter/'.
 * @return string Absolute site-relative target path.
 */
function fastsan_legacy_category_target( $rest ) {
	$hubs = array(
		'insikter' => '/insikter/',
	);
	$slug = trim( (string) $rest, '/' );
	if ( '' === $slug ) {
		return '/insikter/';
	}
	// Only the first segment is a term slug; ignore pagination etc.
	$parts = explode( '/', $slug );
	$first = $parts[0];
	if ( isset( $hubs[ $first ] ) ) {
		return $hubs[ $first ];
...
add_action( 'template_redirect', static function () {
	...
	if ( isset( $map[ $path ] ) ) {
		...
		// 1.2.0 — old category archive base under /sv/ -> editorial hub.
		$target = fastsan_legacy_category_target( substr( $path, strlen( '/sv/category/' ) ) );
	} elseif ( 0 === strpos( $path, '/category/' ) ) {
		// 1.2.0 — pre-cutover category base without language prefix -> same target.
		$target = fastsan_legacy_category_target( substr( $path, strlen( '/category/' ) ) );
